...
export default function Sidebar(props) {
  ...
  function handleRoutesLinks(routes, parentKey = "", mobile) {
    return (
      <List className={classes.list} style={{ width: parentKey.length ? "90%" : "100%", marginLeft: "auto" }}>
        {routes.map((prop, key) => {
          key = parentKey + "_" + key
          if (prop.path && activeRoute(prop)) window.document.title = siteName + " " + (prop.tabTitle || prop.name)
          ...
          if (prop.type === "title")
            return (
              <Typography key={key} className={classes.title}>
                {prop.text}
              </Typography>
            )
          var listItemClasses
          ...
          return (
            <div key={key} style={prop.bottom ? { position: "absolute", bottom: prop.bottom, left: 0, right: 0 } : {}}>
              <NavLink

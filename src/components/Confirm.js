    ...
    return (
      <div>
        ...
        >
          ...
          {confirm.subtitle && <DialogContent>
            <DialogContentText id="alert-dialog-description">
              {confirm.subtitle}
            </DialogContentText>
          </DialogContent>}
          <DialogActions>
            {!confirm.buttons && <><Button onClick={_ => this.onBtnClicked(confirm.onDisagree)} color="primary">
              Annuler
            </Button>
              <Button onClick={_ => this.onBtnClicked(confirm.onAgree)} color="primary" autoFocus>
                Valider
          </Button></>}
            {confirm.buttons && confirm.buttons.map((btn, id) => <Button key={id} onClick={_ => this.onBtnClicked(btn.onClick)} color="primary">
              {btn.txt}
            </Button>)}
          </DialogActions>
        </Dialog>

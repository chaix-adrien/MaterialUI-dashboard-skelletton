...
class FormEditor extends React.Component {
  ...
  componentDidMount() {
    if (!this.isNew) dispatch(Actions.getUser(this.props.match.params.id))
    else dispatch(Actions.getUser())
  }
  ...
  getFormSchema = () => {
    ...
    function getDefault(key) {
      if (isNew || !editUser) return { default: undefined }
    ...
    return {
      ...
      properties: {
        ...
        role: {
          ...
          readOnly,
          enum: [1, 2, 3],
          enumNames: ["Formateur", "Administrateur", "Responsable"],
        },
      },
    }
  }
    ...
    return (
      <div className={c.container}>
        <Card className={c.formCard + " formauto " + (this.readOnly ? " formReadOnly" : "")}>
          ...
          <CardBody>
            <Form schema={this.getFormSchema()} ref={e => (this.form = e)} noValidate={true} onSubmit={this.onSubmit} onError={console.log}>
              <Button
                color='primary'
                type='submit'
                variant='contained'
                style={{ visibility: this.readOnly ? "hidden" : "visible", margin: "auto", display: "flex", marginBottom: 15 }}
                className={this.readOnly ? "hidden" : ""}>
                Valider
              </Button>
            </Form>
          </CardBody>
